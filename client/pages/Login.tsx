...
export default function Login() {
  ...
  return (
    <div className="container mx-auto py-16 max-w-md">
      ...
      <p className="text-muted-foreground mt-2">Sign in to manage your listings and purchases.</p>
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Email (college email recommended)</label>
          <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@college.edu" />
        </div>
        <div className="flex gap-2">
          <Button type="submit">Sign in</Button>
          <Button variant="outline" onClick={() => { setName(""); setEmail(""); }}>Reset</Button>
        </div>
      </form>
    </div>
  );
